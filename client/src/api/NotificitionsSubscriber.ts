...
export const subscribeToNotification = async (userId: string, notificationId: number): Promise<Notification[]> => {
  ...
  const response = await fetch("http://localhost:3000/notifications/user-notification", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
...
export const unsubscribeToNotification = async (userId: string, notificationId: number): Promise<Notification[]> => {
  ...
  const response = await fetch(`http://localhost:3000/notifications/user-notification?user_id=${userId}&notification_id=${notificationId}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json"
      },
